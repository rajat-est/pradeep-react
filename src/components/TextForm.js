...
const TextForm = (props) => {
  ...
  return (
    <div className="container">
      <div
        className={`form_textarea ${props.mode === "light" ? "grey" : "white"}`}
      >
        ...
        <textarea
          ...
          id="textarea"
          rows="20"
          className={`textarea ${props.mode === "light" ? "grey" : "white"}`}
          value={text}
          onChange={Textchange}
        ></textarea>
      ...
      <div className="coventcount">
        <p className={`${props.mode === "light" ? "grey" : "white"}`}>
          total words {wordCount} total latters {latterCount}
        </p>
        <p className={`${props.mode === "light" ? "grey" : "white"}`}>
          {" "}
          total time to read this Pragraph {0.008 * text.split(" ").length}
        </p>
      </div>
